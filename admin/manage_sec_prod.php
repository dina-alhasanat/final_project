<?php
    ...
    if (isset($_POST['submit'])) 
    {
        ...
        // No error was faced! Phew!
            ...
            $query = "insert into secprod(secprod_name , secprod_price  , secprod_image  , cat_id)
                       values('$name' , $price  , '$image_name'  , $seccate_id)";
            
            // echo $query; die;

            mysqli_query($conn, $query);
        
            header('location:manage_sec_prod.php');
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        <div class="cmp-tb-hd">
                            <h2>secondary Product information</h2>
                                ...
                                <div class="nk-int-st">
                                   <select name='cat_id' class="form-control input-sm">
                                            <?php
                                                ...
                                                {
                                                    echo "<option value ='{$category['seccate_id']}'>{$category['seccate_name']}</option>";
                                                }
                        ...
                        <div class="bsc-tbl-cls">
                            <table class="table table-cl">
                                ...
                                <tbody>
                                  ...
                                  $query="select * from secprod inner join seccate on seccate.seccate_id=secprod.cat_id";
                                        $result = mysqli_query($conn , $query); 
                                        while ($product= mysqli_fetch_assoc($result)) {
                                            echo "<tr class='active'>";
                                            echo "<td>{$product['secprod_id']}</td>";
                                            echo "<td>{$product['secprod_name']}</td>";
                                            echo "<td>{$product['secprod_price']}</td>";
                                            echo "<td><img src='uploads/{$product['secprod_image']}' width='200px' height='200px'></td>";
                                            echo "<td>{$product['seccate_name']}</td>";
                                            echo "<td><a href='edit_pro.php ? id={$product['secprod_id']}' class='btn btn-info'>Edit</a></td>";
                                            echo "<td><a href='delete_pro.php ? id={$product['secprod_id']}' class='btn btn-danger'>Delete</a></td>";
                                            echo "</tr>";
                                        }
